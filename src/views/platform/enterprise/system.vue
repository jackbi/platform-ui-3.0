 ...
 * @Version: 1.0
 * @Author: wenbin
 * @Date: 2021-09-06 11:08:12
 * @LastEditors: wenbin
 * @LastEditTime: 2022-05-20 16:35:08
 * @FilePath: /magus-platform-ui-3.0/src/views/platform/enterprise/system.vue
    ...
    <magus-add-template :title="$route.meta.title">
      <template #main>
        <el-form
          ...
          :model="systemInfo"
        >
          <el-form-item :label="$t('platform.company.loginBg')">
            <div v-if="backgroundImage" class="image-show">
              <el-image
                class="image-show-picture"
                :src="$filters.getImage(backgroundImage)"
              />
            ...
            >
              ...
              <template #tip>
                <pre class="el-upload__tip">{{
                  $t('platform.company.loginBgUploadTips')
                }}</pre>
              </template>
            </el-upload>
          </el-form-item>
          ...
          <el-form-item :label="$t('platform.system.system_login_title1')">
            ...
            <el-row v-show="!systemInfo.showLoginLogo">
              <el-input
          ...
          <el-form-item :label="$t('platform.system.system_title1')">
            ...
            <el-row v-show="!systemInfo.showLogo">
              <el-input
...
export default defineComponent({
  ...
  setup() {
    ...
    const methods = {
      ...
      changeBackgroundImage(file, fileList) {
        if (fileList.length > 0) {
          state.backgroundImageFile = [fileList[fileList.length - 1]]; // 这一步，是 展示最后一次选择的文件
        }
        methods.imagePreview(file.raw, 'backgroundImage');
        state.uploadBackground = true;
      },
      /**
       * @description: 选择logo
      ...
      goSave() {
        ...
        if (state.backgroundImageFile && state.backgroundImageFile.length) {
          formData.append('background', state.backgroundImageFile[0].raw);
        }
        ...
        formData.append('updateBackground', state.uploadBackground);

        proxy.$magusCloudApi.apiConfig
          .post(enterpriceApi.saveTenantCustomsCurrent, formData)
          .then(() => {
            proxy.$message({
              type: 'success',
            ...
            state.loading = false;
            methods.getEnterpriceSys();
            state.uploadBackground = false;
            state.uploadLogo = false;
      ...
      getEnterpriceSys() {
        proxy.$magusCloudApi.apiConfig
          .get(enterpriceApi.getTenantCustomsCurrent)
          .then((res) => {
            Object.keys(state.systemInfo).forEach((item) => {
              state.systemInfo[item] =
                proxy.$magusCloudApi.judgeUtils.isBoolean(res[item])
                  ? res[item]
                  : res[item] || '';
            });
            ...
            state.backgroundImage = res.background ? res.background : '';
